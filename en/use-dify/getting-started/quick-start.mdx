...
This document has been modified to test the refactored translation system. The system now uses `zh`/`ja` language codes (instead of `cn`/`jp`) and config-driven language references throughout. This modification should trigger context-aware translation using git diff analysis.
