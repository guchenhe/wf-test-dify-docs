...
2. イテレーションノード内に、LLMノードを追加して設定します：
   ...
   3. システムプロンプトフィールドに、以下を貼り付けます：
      ...
      ````
3. Identity Styleノードの後に、別のLLMノードを追加して設定します：
...
このドキュメントは、リファクタリングされた翻訳システムをテストするために変更されました。システムは現在、`zh`/`ja`言語コード（`cn`/`jp`の代わり）と設定駆動の言語参照を全体的に使用しています。この変更により、git diff分析を使用したコンテキスト認識翻訳がトリガーされます。